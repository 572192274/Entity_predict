...
if __name__=='__main__':
    ...
    print('数据处理开始...')
    print('--------------------------------------------------------------')
    for diglog in tqdm(test_data):
        diglog_list=diglog['history']
        sentence='，'.join(diglog_list)
        sentence_dict['Sentence'] = sentence
        source = {'history': sentence, 'next_sym': []}
        ...
        for label_id,prob in enumerate(prediction_data['probs']):#prediction_data是一个字典，预测的sigmoid值，160个
            ...
            if prob>0.5:
                ...
                if 0<=label_id<=61:
                    ...
                    sentence_dict['Symptom']=sym_list
                elif 62<=label_id<=123:
                    med_list.append(data_keys[label_id])
                    sentence_dict['Medicine']=med_list
                elif 124<=label_id<=143:
                    test_list.append(data_keys[label_id])
                    sentence_dict['Test']=test_list
                elif 144<=label_id<=147:
                    arr_list.append(data_keys[label_id])
                    sentence_dict['Attribute']=arr_list
                elif 148<=label_id<=159:
                    dis_list.append(data_keys[label_id])
                    sentence_dict['Disease']=dis_list
        if 'Symptom' not in list(sentence_dict.keys()):
            sentence_dict['Symptom']=[]
        if 'Medicine' not in list(sentence_dict.keys()):
            sentence_dict['Medicine']=[]
        if 'Test' not in list(sentence_dict.keys()):
            sentence_dict['Test']=[]
        if 'Attribute' not in list(sentence_dict.keys()):
            sentence_dict['Attribute']=[]
        if 'Disease' not in list(sentence_dict.keys()):
            sentence_dict['Disease']=[]
        single_list.append(sentence_dict)
        sentence_dict={}
        # print(single_list)
        num=num+1
        # print(num)
        ...
        single_list=[]
    output=open('processed_data/PhasesBTest_all.pk','wb')
