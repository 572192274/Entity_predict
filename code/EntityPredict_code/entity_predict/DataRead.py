...
class MacroF(Metric):
    ...
    def __call__(self, predict, ground_true):
        ...
        true_positive_index = (predict == ground_true).float() * (ground_true == 1).float()
        self._true_positive += torch.sum(true_positive_index, 0).squeeze(0)
        self._ground_truth += torch.sum(ground_true, 0).squeeze(0).float()
        self._predict += torch.sum(predict, 0).squeeze(0).float()
